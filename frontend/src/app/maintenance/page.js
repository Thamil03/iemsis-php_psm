...
export default function MaintenancePage() {
  ...
  const fetchMaintenanceRecords = async () => {
    try {
      const response = await fetch("http://188.166.227.229/backend/maintenance/fetch_all.php");
    ...
    if (window.confirm("Are you sure you want to delete this maintenance record?")) {
      try {
        const response = await fetch(
          `http://188.166.227.229/iemsis-php/maintenance/MaintenanceController.php?action=delete&id=${id}`,
          {
            method: "DELETE",
            credentials: "include",
          }
  ...
  const handleSave = async () => {
    ...
    const url = editingId
      ? `http://188.166.227.229/backend/maintenance/update.php?id=${editingId}`
      : "http://188.166.227.229/backend/maintenance/add.php";
