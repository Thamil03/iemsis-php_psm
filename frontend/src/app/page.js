...
export default function Home() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const res = await fetch("http://localhost/iemsis-php/notifications/NotificationsController.php");
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`http://localhost/iemsis-php/equipment/EquipmentController.php?search=${searchNumber}`);
      const data = await res.json();
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost/IEMSIS-PHP/user/UserController.php?action=forgot_password', {
        method: 'POST',
        headers: {
