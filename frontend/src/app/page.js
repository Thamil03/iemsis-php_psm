...
export default function Home() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const res = await fetch("http://157.230.245.190:8001/notifications/NotificationsController.php");
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`http://157.230.245.190:8001/equipment/EquipmentController.php?search=${searchNumber}`);
      const data = await res.json();
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await fetch('http://157.230.245.190:8001/user/UserController.php?action=forgot_password', {
        method: 'POST',
        headers: {
