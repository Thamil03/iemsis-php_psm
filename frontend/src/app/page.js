...
export default function Home() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`http://157.230.245.190:8000/equipment/EquipmentController.php?search=${searchNumber}`);
      const data = await res.json();
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await fetch('http://157.230.245.190:8000/user/UserController.php?action=forgot_password', {
        method: 'POST',
        headers: {
