...
const AdminPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch(
        "http://157.230.245.190/user/UserController.php?action=fetch_all",
        { credentials: "include" }
      );
  ...
  const fetchDropdownData = async () => {
    try {
      const res = await fetch(
        "http://157.230.245.190/equipment/EquipmentController.php?action=fetch_dropdowns"
      );
      const data = await res.json();
  ...
  const handleAddDropdownOption = async () => {
    ...
    try {
      const res = await fetch(
        "http://157.230.245.190/equipment/EquipmentController.php?action=add_option",
        {
          method: "POST",
          credentials: "include",
  ...
  const handleDeleteOption = async (type, value) => {
    try {
      const res = await fetch(
        "http://157.230.245.190/equipment/EquipmentController.php?action=delete_option",
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleResetPassword = async (userId) => {
    if (window.confirm("Are you sure you want to reset this user's password to Default123?")) {
      try {
        const res = await fetch(`http://157.230.245.190/user/UserController.php?action=reset_password`, {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleToggleActive = async (user) => {
    ...
    try {
      const res = await fetch("http://157.230.245.190/user/UserController.php?action=update_status", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      const res = await fetch("http://157.230.245.190/user/UserController.php?action=add", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDeleteUser = async (user) => {
    if (window.confirm(`Are you sure you want to delete user ${user.name}? This action cannot be undone.`)) {
      try {
        const res = await fetch("http://157.230.245.190/user/UserController.php?action=delete", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://157.230.245.190/user/UserController.php",
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await fetch(
          `http://157.230.245.190/user/UserController.php?action=delete&id=${id}`,
          {
            method: "DELETE",
            credentials: "include",
          }
  ...
  const fetchLogs = async () => {
    try {
      setLogsLoading(true);
      const url = new URL('http://157.230.245.190/logs/LogController.php');
  ...
  const fetchResetRequests = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://157.230.245.190/user/UserController.php?action=fetch_reset_requests', {
        method: 'GET',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleResetRequest = async (requestId, action) => {
    try {
      const response = await fetch('http://157.230.245.190/user/UserController.php?action=handle_reset_request', {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white p-6 rounded-lg w-96 shadow-lg relative z-10 border border-gray-200 animate-fadeIn">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                fetch("http://157.230.245.190/user/UserController.php?action=update", {
                  method: "POST",
                  credentials: "include",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
